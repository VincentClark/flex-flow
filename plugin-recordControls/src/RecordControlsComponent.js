...
export class RecordControlsComponent extends React.Component {
  ...
  makeRequest = t => {
    ...
    let requestURL = `https://wine-labradoodle-8216.twil.io/recordingControls?`;
    requestURL += `setStatus=${t}`;
    requestURL += `&recordingSid=${this.state.recordingSid}`;
    requestURL += `&callSid=${this.props.task.attributes.conference.participants.customer}`;
    //requestURL += `&callSid=${this.props.task.conference.participants[1]._callSid}`;
